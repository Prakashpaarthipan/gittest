...
{
	//echo $_SESSION['username1'] .'!!!'.$_SESSION['password1'] .'@@@'. $_SESSION['host_db1'];
	//include("config.php");
...
function select_softquery($sqlqry_selectsoft,$brn,$mode)
{
	$branch_code=$brn;   //  $branch_code will used in config.php for  branch repesentation('Local',1,2,3...17)
	$mode_detail=$mode;  //  $mode_detail will used in config.php for  mode repesentation('TCS','TEST','KTM')
	include("config_old.php");
	//echo $_SESSION['allbrnusername']."%%".$_SESSION['allbrnpassword']."%%".$_SESSION['allbrnhost_db'];
	$softconnect_crmdb = oci_connect($_SESSION['allbrnusername'], $_SESSION['allbrnpassword'], $_SESSION['allbrnhost_db'], 'AL32UTF8');
	 
	try {
		$parseqrysoft = oci_parse($softconnect_crmdb, $sqlqry_selectsoft); // Parse the connection and DB.
		oci_execute($parseqrysoft); // Execute the query
		$res_qrysoft = array();
		while(($row_qryksreq = oci_fetch_array($parseqrysoft, OCI_BOTH)) != false) {
			//print_r($row_qrycrm);
			$res_qrysoft[] = $row_qryksreq;
		}
		return $res_qrysoft;
	} 
	catch(Exception $e) {
...
function insert_softquery($field_valuesoft, $tbl_namesoft,$brncode,$brnmode)  
{
	
		$branch_code=$brncode;  //  $branch_code will used in config.php for  branch repesentation('Local',1,2,3...17)
		$mode_detail=$brnmode;  //  $mode_detail will used in config.php for  mode repesentation('TCS','TEST','KTM')
		include("config_old.php");
		$connect_allbrndb = oci_connect($_SESSION['allbrnusername'], $_SESSION['allbrnpassword'], $_SESSION['allbrnhost_db'], 'AL32UTF8');
	
	try {
		$key_valueksreq = array_keys($field_valuesoft);
		$org_valuesoft = array_values($field_valuesoft);
		$key_valuesoft = array_keys($field_valuesoft);
		for($iiksreq = 0; $iiksreq < count($field_valuesoft); $iiksreq++) {
			$expld = explode("~~",$org_valuesoft[$iiksreq]);
			if($expld[1] != '')
			{
				$kvlksreq .= $key_valuesoft[$iiksreq].", ";
				$kyvlksreq .= "to_date(:".$key_valuesoft[$iiksreq].", '".$expld[0]."'), ";
				$kkvlksreq[] = "".$key_valuesoft[$iiksreq]."";
			} else {
				$kvlksreq .= $key_valuesoft[$iiksreq].", ";
				$kyvlksreq .= ":".$key_valuesoft[$iiksreq].", ";
				$kkvlksreq[] = "".$key_valuesoft[$iiksreq]."";
			}
		...
		$sql_insertksreq ="insert into ".$tbl_namesoft." (".$kvlksreq.") values (".$kyvlksreq.")";
		$res_insertksreq=oci_parse($connect_allbrndb, $sql_insertksreq);
		
		for($ij = 0; $ij < count($field_valuesoft); $ij++) {
			$expld = explode("~~",$org_valuesoft[$ij]);
			if($expld[1] != '')
			{
				oci_bind_by_name($res_insertksreq, $kkvlksreq[$ij], $expld[1]);
			} else {
				oci_bind_by_name($res_insertksreq, $kkvlksreq[$ij], $org_valuesoft[$ij]);
			}
			//oci_bind_by_name($res_insertksreq, $kkvlksreq[$ij], $org_valuesoft[$ij]);
		}
		$res_executeksreq=oci_execute($res_insertksreq);
...
$where_conditioncrms = 'EMPCODE = 14442';
echo update_ksreqquery($field_valuecrm, $tbl_name, $where_conditioncrms); */
function update_softquery($field_valuecrmsksreq, $tbl_namesksreq, $where_conditionksreq)
{
	if($_SESSION['dbassign'] == '') {
		include("config.php");
	}
...
/* Delete Query Started - Delete is not required here. Just update the deleted indicator 'Y' using update_query */
//echo delete_ksreqquery('delete from group_reg where EMPCODE = 1');
function delete_softquery($delete_qrycrmksreq) 
{
	if($_SESSION['dbassign'] == '') {
		include("config.php");
	}

...
function select_softquery($sqlqry_selectsoft,$brn,$mode)
{
	...
	// exit;
		...
		$source_allbrn = $allbrn->Get_Local_Conn($brn, $mode);
	//echo "****";
	//Echo $source_allbrn;
		...
		$exp_tcs_allbrn = explode(", ",$connection_allbrn);
		$allbrn_det=explode(",",$exp_tcs_allbrn[0]);
		$allbrn_host=explode("/",$allbrn_det[2]);
		
		//echo $allbrn_det[0]."**".$allbrn_det[1]."**".$allbrn_det[2];
		$connect_allbrndb = oci_connect($allbrn_det[0], $allbrn_det[1], $allbrn_det[2]); 
		
		$parseqrysoft = oci_parse($connect_allbrndb, $sqlqry_selectsoft); // Parse the connection and DB.
		oci_execute($parseqrysoft); // Execute the query
		$res_qrysoft = array();
		while(($row_qryksreq = oci_fetch_array($parseqrysoft, OCI_BOTH)) != false) {
			//echo "&&&&";print_r($row_qryksreq);
